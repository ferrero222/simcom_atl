 ...
 ** \retval true - proc started, false - smthg is wrong
 ******************************************************************************/
bool atl_mdl_gprs_socket_connect(char* mode, char* ip, char* port, atl_entity_cb_t cb)
{
  char cipstart[128] = {0}; 
  snprintf(_cmd, sizeof(_cmd), "%sAT+CIPSTART=\"%s\",\"%s\",\"%s\"%s", ATL_CMD_SAVE, mode, ip, port, ATL_CMD_CRLF); 
  atl_item_t items[] = //[REQ][PREFIX][FORMAT][RPT][WAIT][STEPERROR][STEPOK][CB][...##VA_ARGS]
  { 
    ATL_ITEM("AT+CIPSTATUS"ATL_CMD_CRLF, "STATE: IP STATUS|STATE: TCP CLOSED",           NULL, 2, 25,  0, 1, NULL),
    ATL_ITEM(cipstart,                                           "CONNECT OK",           NULL, 5, 160, 0, 1, NULL),
 ...
 ** \retval true - proc started, false - smthg is wrong
 ******************************************************************************/
bool atl_mdl_gprs_socket_send_recieve(char* data, atl_entity_cb_t cb)
{
  DBC_REQUIRE(101, atl_is_init());
  if(atl_mdl_gprs_send_len && atl_mdl_gprs_send_len < strlen(data)) return false;
  size_t size = strlen(ATL_CMD_SAVE) + strlen(_data) + strlen(ATL_CMD_CTRL_Z) +1; 
  char cipsend[64] = {0}; 
  char* datacmd = (char*)tlsf_malloc(atl_user_init.atl_tlsf, size);
  if(!datacmd) 
  {
  ...
  snprintf(cipsend, sizeof(cipsend), "%sAT+CIPSEND=%d%s", ATL_CMD_SAVE, strlen(data), ATL_CMD_CRLF); 
  snprintf(datacmd, size, "%s%s%s", ATL_CMD_SAVE, data, ATL_CMD_CTRL_Z); 
  atl_item_t items[] = //[REQ][PREFIX][FORMAT][RPT][WAIT][STEPERROR][STEPOK][CB][...##VA_ARGS]
  {
    ATL_ITEM("AT+CIPSTATUS"ATL_CMD_CRLF, "STATE: CONNECT OK", NULL, 2, 25,  0, 1, NULL),
  ...
  tlsf_free(atl_user_init.atl_tlsf, datacmd);
  if(!atl_enqueue(items, sizeof(items)/sizeof(items[0]), cb)) return false;
  return true;
}
